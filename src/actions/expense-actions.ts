...
interface ExpenseInput {
  ...
  splitEntries: [string, number][]
}

async function resolveExpenseInput(
  formData: FormData,
  user: { email?: string | null; name?: string | null; image?: string | null }
): Promise<{ error: string } | ExpenseInput> {
  ...
  return { dbUser, groupId, title, amount, currency, paidBy, date, note, category, splitEntries }
}
...
export async function createExpense(formData: FormData): Promise<{ error?: string }> {
  ...
  try {
    const resolved = await resolveExpenseInput(formData, user)
    if ('error' in resolved) return resolved

    const { dbUser, groupId, title, amount, currency, paidBy, date, note, category, splitEntries } = resolved
...
export async function updateExpense(formData: FormData): Promise<{ error?: string }> {
  ...
  try {
    ...
    const { groupId, title, amount, currency, paidBy, date, note, category, splitEntries } = resolved
